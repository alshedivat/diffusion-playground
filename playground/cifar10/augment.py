"""Data augmentation for training denoising diffusion on CIFAR10.

All the code below is copied verbatim (up to formatting) from:
https://github.com/NVlabs/edm/blob/62072d2612c7da05165d6233d13d17d71f213fee/training/augment.py.
...
def matrix(*rows, device=None):
    ...
    elems = [
        x if isinstance(x, torch.Tensor) else constant(x, shape=ref[0].shape, device=ref[0].device)
        for x in elems
    ]
...
class AugmentPipe:
    ...
    def __call__(self, images):
        ...
        if self.translate_int > 0:
            ...
            w = torch.where(
                torch.rand([1, N, 1, 1, 1], device=device) < self.translate_int * self.p,
                w,
                torch.zeros_like(w),
            )
        ...
        if self.scale > 0:
            ...
            w = torch.where(
                torch.rand([N], device=device) < self.scale * self.p, w, torch.zeros_like(w)
            )
        ...
        if self.rotate_frac > 0:
            ...
            w = torch.where(
                torch.rand([N], device=device) < self.rotate_frac * self.p, w, torch.zeros_like(w)
            )
        ...
        if self.aniso > 0:
            ...
            w = torch.where(
                torch.rand([N], device=device) < self.aniso * self.p, w, torch.zeros_like(w)
            )
            r = torch.where(
                torch.rand([N], device=device) < self.aniso_rotate_prob, r, torch.zeros_like(r)
            )
        ...
        if self.brightness > 0:
            w = torch.randn([N], device=device)
            w = torch.where(
                torch.rand([N], device=device) < self.brightness * self.p, w, torch.zeros_like(w)
            )
        ...
        if self.contrast > 0:
            w = torch.randn([N], device=device)
            w = torch.where(
                torch.rand([N], device=device) < self.contrast * self.p, w, torch.zeros_like(w)
            )
        ...
        if self.hue > 0:
            ...
            w = torch.where(
                torch.rand([N], device=device) < self.hue * self.p, w, torch.zeros_like(w)
            )
